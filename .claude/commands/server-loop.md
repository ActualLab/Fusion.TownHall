...
The loop owns the TownHall server process — don't start `dotnet run`
...
- **Docker/WSL** (`AC_OS` = `Linux in Docker` / `Linux on WSL`): the PID
  ...
  nothing in your PID namespace (probe `http://localhost:5136` for a hint
...
1. `dotnet-build` — `dotnet build -c <Configuration> TownHall.slnx`
2. `server-run` — runs the built server binary from
   `artifacts/bin/TownHall.Host/<configuration>/` on `http://localhost:5136`
   (override with `-Port`).

A watchdog probes `http://localhost:5136` every 15 s (first probe 10 s
...
- **`http://localhost:5136`** — the server URL (no NGINX, no TLS in this
...
echo
echo "=== reachability ==="
curl -s -o /dev/null -w "http://localhost:5136 -> HTTP %{http_code}\n" --max-time 3 http://localhost:5136 \
    || echo "http://localhost:5136 -> DOWN"
```
