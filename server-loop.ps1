# server-loop.ps1
# Edit-run-restart loop for the TownHall server.
#
# Runs two steps in sequence:
#   1) dotnet-build  - dotnet build -c $Configuration TownHall.slnx
#   2) server-run    - runs the built TownHall server on http://localhost:$Port
#
...
while ($true) {
    ...
    & dotnet build -c $Configuration TownHall.slnx *> $dotnetBuildLog
